  ...
  return (
    <button
      ...
      className={clsx(
        ...
        card.isShiny ? 'border-amber-300/70 ring-1 ring-amber-200/40' : 'border-white/10',
        selectable && 'cursor-pointer',
        selected && 'border-orange-300 ring-2 ring-orange-300/70',
      )}
    >
      <div className="relative aspect-[3/4] overflow-hidden">
        <img src={card.imageUrl} alt={card.itemName} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="font-serif text-3xl text-white">{card.itemName}</p>
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm text-slate-300">
          <div>
            ...
            <p>{Math.max(6, 18 - Math.min(card.editionNumber, 3) * 3) + (card.isShiny ? 15 : 0)}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p>{card.previousOwners.length} keepers</p>
          </div>
        </div>
